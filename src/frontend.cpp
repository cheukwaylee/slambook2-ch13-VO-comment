...
namespace myslam
{
    ...
    bool Frontend::StereoInit()
    {
        ...
        //根据左目特征在右目中找对应，虽然左目提取时特征点数量能够保证，但匹配过程则无法确保
    ...
    bool Frontend::Track()
    {
        ...
        {
            // 以【上一帧位姿】乘以【上上帧与上一帧之间的相对运动】作为这次位姿的估计初值: taylor
        ...
        //从上一帧跟踪到当前帧，跟踪到了多少个特征，num_track_last可理解为从上一帧跟踪过来到当前帧,跟踪到了的特征数目。
        // 此处光流法匹配上一帧与当前帧，并返回光流法匹配到的点的数量
        int num_track_last = TrackLastFrame();

        // 在此处修正当前帧的位姿估计，并返回追踪成功点数量（在光流匹配成功基础上，更信任的点）
        tracking_inliers_ = EstimateCurrentPose();
        //因为前面已经知道了一个current_pose，所以可以根据这个pose初值判断inliers和outliers
    ...
    int Frontend::FindFeaturesInRight()
    {
        ...
        for (size_t i = 0; i < status.size(); ++i)
        {
             ...
             * 它是一种“整型”类型，里面保存的是一个整数，就像int, long那样。这种整数用来记录一个大小(size)。size_t的全称应该是size type，
             就是说“一种用来记录大小的数据类型”。通常我们用sizeof(XXX)操作，这个操作所得到的结果就是size_t类型。
    ...
    bool Frontend::BuildInitMap()
    {
        ...
        //看到这里应该记得，对Frame也有一个pose，Frame里面的pose描述了固定坐标系（世界坐标系）和某一帧间的位姿变化，
        ...
        for (size_t i = 0; i < current_frame_->features_left_.size(); ++i)
        {
            ...
            //左目右目的像素坐标都转为相机坐标，存到一个vector里面，下一次循环中将重置一个含有两个元素的新vector
    ...
    int Frontend::TrackLastFrame()
    {
        ...
        for (auto &kp : last_frame_->features_left_)
        {
            ...
            // MapPoint的形成是需要左目和同一帧的右目中构成对应关系才可以，有些左目中的feature在右目中没有配对，
            //就没有Mappoint，但是没有Mappoint却不代表这个点是一个outlier
            ...
            // 否则当前帧的光流法初值就是上一帧的关键点坐标
            else
            {
                //对于建立了Mappoint的特征点而言 kps_current的初值是kps_last
                kps_last.push_back(kp->position_.pt);
        ...
        Mat err;
        // OpenCV光流法函数解析
        // calcOpticalFlowPyrLK(    匹配图1，匹配图2，
        ...
        for (size_t i = 0; i < status.size(); ++i)
        {
            if (status[i]) // 匹配成功 // status[i]=true则说明跟踪成功有对应点，false则跟踪失败没找到对应点
            {
                // 关键点代表的区域直径大小为7
    ...
    int Frontend::EstimateCurrentPose()
    {
        ...
        //明确一下这个过程中我们要优化的变量是什么，这里我们认为地图的3D位置是不用再去优化的（想优化也可以），
        //所以这个过程中只需要优化R，t
        ...
        for (int iteration = 0; iteration < 4; ++iteration)
        {
            ...
            {
                auto e = edges[i];
                if (features[i]->is_outlier_) // 特征点本身就是异常点，计算重投影误差s
                {
                    e->computeError();
                }
                ...
                if (iteration == 2)
                {
                    e->setRobustKernel(nullptr); // 最后一次循环时去掉鲁棒核函数
                }
            }
        }
        // 异常点/正常点记录到日志
        ...
        for (auto &feat : features)
        {
            if (feat->is_outlier_)
            {
                ...
                feat->is_outlier_ = false; // maybe we can still use it in future？？？？？？这里这么搞是怎么个意思, 竹曼好像懂了
            }
        }
    ...
    int Frontend::TriangulateNewPoints() //这个函数其实与BuildInitMap差不多
    {
        ...
        for (size_t i = 0; i < current_frame_->features_left_.size(); ++i)
        {
            // expired()表示指针失效与否,expired()为true等价于use_count() == 0。所以true表示关联的路标点不存在，false表示关联的路标点存在。
            // expired()函数用于检查智能指针指向的对象是否为空
            // 即：检查当前帧是否有对应的地图点，是否有对应的右图匹配点，若都满足，则三角化
                ...
                current_frame_->features_right_[i] != nullptr)
            {
                ...
                { //如果三角化流程运行成功且得到的深度值pworld[2]大于0有意义
                    ...
                    // 设置新地图点坐标，拥有的特征点，建立当前帧所有特征点与地图点的对应关系
                    // 为地图点类添加特征成员变量 //增加mappoint类中的对应的那个feature（左右目）
